...
phonecatControllers.controller('UsersListCtrl', ['$scope', '$http','$window',
  function($scope, $http, $window) {
    ...
    $http.get('http://localhost:1337/users', { headers: { 'Authorization': 'Bearer ' + localStorage.getItem("token")  }}).then(function successCallback(response) {
    ...
    $scope.removeUser = function (userId) {
    	...
    	var deleteUser = $window.confirm('Are you absolutely sure you want to delete?');
    	 if (deleteUser) {
    	 	...
    	 	$http.post('http://localhost:1337/users/destroy?id='+userId).then(function successCallback(response) {
  ...
  function($scope, $http, $window,$routeParams) {
    ...
    $http.get('http://localhost:1337/albums?userown='+$routeParams.userId, { headers: { 'Authorization': 'Bearer ' + localStorage.getItem("token")  }}).then(function successCallback(response) {
...
phonecatControllers.controller('UserAlbumMissingCardCtrl', ['$scope', '$http','$window','$routeParams',
  function($scope, $http, $window,$routeParams) {
    ...
    $http.get('http://localhost:1337/missingcards?albumcard='+$routeParams.albumId).then(function successCallback(response) {
    ...
    $scope.addMissingCard = function (card,textdescription) {
	  console.log("OK users add missing cards");
		...
		console.log($routeParams.albumId);
  	//$http.defaults.headers.common.Authorization = 'Bearer ' + localStorage.getItem("token");
	  $http.post('http://localhost:1337/missingcards/create?card='+card+'&textdescription='+textdescription+'&albumcard='+$routeParams.albumId).then(function successCallback(response) {
    ...
    $scope.findMissingCard = function (cardId) {
  	...
  	//$http.defaults.headers.common.Authorization = 'Bearer ' + localStorage.getItem("token");
	  $http.post('http://localhost:1337/missingcards/update/'+cardId+'?foundby='+localStorage.getItem("usr")).then(function successCallback(response) {
    ...
    $scope.removeMissingCard = function (cardId) {
	  console.log("OK users REMOVE missing cards");
		...
		console.log($routeParams.albumId);
  	//$http.defaults.headers.common.Authorization = 'Bearer ' + localStorage.getItem("token");
	  $http.post('http://localhost:1337/missingcards/destroy?id='+cardId).then(function successCallback(response) {
...
phonecatControllers.controller('AlbumsListCtrl', ['$scope', '$http','$window',
  function($scope, $http, $window) {
  	console.log("OK albums list controller");
		console.log(localStorage.getItem("token"));
    $http.get('http://localhost:1337/albums', { headers: { 'Authorization': 'Bearer ' + localStorage.getItem("token")  }}).then(function successCallback(response) {
    									console.log("OK elenco caricato");
      								$scope.albums = response.data;
	  }, function errorCallback(response) {
